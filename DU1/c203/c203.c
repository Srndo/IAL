...
*/
	...
	if(q == NULL)			//V případě, že funkce dostane jako parametr q == NULL
		queueError(QERR_INIT);			//volejte funkci queueError(QERR_INIT)
	else
	{
		for(int i = 0; i < QUEUE_SIZE; i++)			//všechny hodnoty v poli q->arr nastavte na '*'
			q->arr[i] = '*';
...
*/
	...
	return (index + 1) % QUEUE_SIZE;		//vdaka % nepretecieme za QUEUE_SIZE
...
*/
	...
	return (q->f_index == q->b_index) ? 1 : 0;	//ak je fronta prazdna hodnota f_index a b_index su totozne
}
...
*/
	...
	return (nextIndex(q->b_index) == q->f_index)? 1 : 0; // ak je fronta plna nextIndex(q->b_index) "pretecie" na tu istu hodnotu ako je f_index
}
...
*/
	...
	if(queueEmpty(q))					//podmienka ak je fronta prazdna
		queueError(QERR_FRONT);
	else
		*c = q->arr[q->f_index];		//nahra do premennej c znak zo zaciatku fronty na ktory ukazuje f_index
}
...
*/
	...
	if(queueEmpty(q))						//podmienka ak je fronta prazdna
		...
		q->f_index = nextIndex(q->f_index);		//odstrani znak zo zaciatku fronty (prerotuje pri preteceni na hodnotu 0)
}
...
*/
	...
	{
		queueFront(q, c);				//nahra do premennej c prvok zo zaciatku fronty
...
*/
	...
	if(queueFull(q))				//podmienka ak je fronta plna
		queueError(QERR_UP);
	else
	{
		q->arr[q->b_index] = c;			//znak c nahra na volny prvok v poli
		q->b_index = nextIndex(q->b_index);		//presunie index volneho prvku (v pripade pretecenia ide znova od zaciatku (0))
	}
}
